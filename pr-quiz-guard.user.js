...
(function () {
  ...
  // ---------------------------------------------------------------------
  // Settings (Tampermonkey menu commands)
  // ---------------------------------------------------------------------

  function getApiKey() {
    return GM_getValue('prQuizGuard:apiKey', '');
  }

  function getModel() {
    return GM_getValue('prQuizGuard:model', DEFAULT_MODEL);
  }
  ...
  GM_registerMenuCommand('Set Model (current: ' + getModel() + ')', () => {
    const next = prompt(
      'Model ID to use for quiz generation/grading:',
      getModel()
    );
    if (next && next.trim()) {
  ...
  // fetch()/XHR calls to the .diff endpoint from page scripts. GM_xmlhttpRequest is
  // dispatched by the browser extension itself, bypassing that entirely.
  function fetchDiff() {
      ...
      GM_xmlhttpRequest({
        ...
        onload(res) {
          if (res.status < 200 || res.status >= 300) {
            reject(new Error('Failed to fetch diff: HTTP ' + res.status));
            return;
          }
        ...
        onerror() {
          reject(new Error('Network error fetching PR diff.'));
        },
      });
    });
  }
  ...
  // diff. Uses the same GM_xmlhttpRequest path as fetchDiff() for the same reason
  ...
  function fetchPrDescription() {
    return new Promise((resolve) => {
      GM_xmlhttpRequest({
        ...
        onload(res) {
          if (res.status < 200 || res.status >= 300) {
            resolve(''); // description is a nice-to-have; never block the quiz on it
            return;
          }
          try {
  ...
  // ---------------------------------------------------------------------
  // Anthropic API calls (via GM_xmlhttpRequest — bypasses page CORS)
  // ---------------------------------------------------------------------

  function callClaude({ system, messages, schema }) {
    const apiKey = getApiKey();
    if (!apiKey) {
      return Promise.reject(
        new Error('No Anthropic API key set. Use the Tampermonkey menu → "Set Anthropic API Key".')
      );
    }
    ...
    return new Promise((resolve, reject) => {
      GM_xmlhttpRequest({
        ...
        onload(res) {
          if (res.status < 200 || res.status >= 300) {
            reject(new Error('Anthropic API error ' + res.status + ': ' + res.responseText));
            return;
          }
          try {
        ...
        onerror() {
          reject(new Error('Network error calling Anthropic API.'));
        },
      });
    });
  }
  ...
  function generateQuiz(diff, title, description) {
    return callClaude({
      system:
        ...
        'Write exactly 3 questions - never fewer, never more. Assume the reviewer can see the ' +
        'diff while answering.',
      messages: [
        {
          role: 'user',
          content:
  ...
  function gradeQuiz(diff, title, description, questions, answers) {
    ...
    return callClaude({
      system:
        'You are a strict but fair code-review comprehension grader. Given a PR title, description, ' +
        'diff, and a reviewer\'s answers to questions about it, grade each answer against what the ' +
        'diff actually does. "pass" = demonstrates real understanding of the specific logic being ' +
        'asked about. "weak" = partially right, vague, hand-wavy, or hedged. "fail" = wrong or does ' +
        'not engage with the actual change. overall_pass is true only if every answer is "pass". ' +
